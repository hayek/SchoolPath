    ...
    if (learningActivitySection) {
        if (poi.hasLearningActivity && poi.learningTasks && poi.learningTasks.length > 0) {
            const tasksHTML = poi.learningTasks.map((task, index) => {
                ...
                return `
                    <div style="margin-top: ${index > 0 ? '20px' : '0'}; padding-top: ${index > 0 ? '20px' : '0'}; border-top: ${index > 0 ? '1px solid #d1e7ff' : 'none'};">
                        <div style="display: flex; align-items: flex-start; gap: 12px;">
                            <span style="color: #007AFF; font-size: 14px; font-weight: 600; flex-shrink: 0;">${index + 1}.</span>
                            <div style="flex: 1;">
                                <h4 style="margin: 0 0 8px 0; color: #007AFF; font-size: 16px;">${taskTitle}</h4>
                                ${taskBody ? `<p style="margin: 0 0 12px 0; color: #333; font-size: 14px; line-height: 1.6; white-space: pre-wrap;">${taskBody}</p>` : ''}
                                ${taskUrl ? `
                                    <a href="${taskUrl}" target="_blank" style="display: inline-flex; align-items: center; gap: 6px; color: #007AFF; text-decoration: none; font-size: 14px; margin-bottom: 8px;">
                                ...
                                ${taskPdfs && taskPdfs.length > 0 ? `
                                    <div style="margin-top: 8px;">
                                        ...
                                        ${taskPdfs.map(pdf => `
                                            <a href="${pdf}" target="_blank" style="display: inline-flex; align-items: center; gap: 6px; color: #007AFF; text-decoration: none; font-size: 13px; margin-left: 12px;">
